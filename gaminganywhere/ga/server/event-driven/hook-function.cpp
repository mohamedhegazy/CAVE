...
TD3D11ClearDepthStencilView pClearDepthStencilView  = NULL;
// --- DXGI ---
...
TCreateDXGIFactory pCreateDXGIFactory1 = NULL;
...
static bool rc;
static int skip;
static bool depth_hooked;
static bool depth_format_set;
static int mode;
static bool
...
// Hook function that replaces the CreateDXGIFactory1() API
DllExport HRESULT __stdcall
hook_CreateDXGIFactory1( REFIID riid, void **ppFactory) {
	//
	//ga_error("Before Create DXGIFactory1\n");
	HRESULT hr = pCreateDXGIFactory1(riid, ppFactory);
	//ga_error("After Create DXGIFactory1\n");
	if (pDXGICreateSwapChain == NULL && riid == IID_IDXGIFactory1 && ppFactory != NULL) {
		uintptr_t* pInterfaceVTable = (uintptr_t*)*(uintptr_t*)*ppFactory;
		//OutputDebugString("[CreateDXGIFactory]");
		pDXGICreateSwapChain = (TDXGICreateSwapChain)pInterfaceVTable[10];   // 10: IDXGIFactory::CreateSwapChain  
		//ga_error("Before Hooking CreateSwapChain\n");
		ga_hook_function("IDXGIFactory1::CreateSwapChain", pDXGICreateSwapChain, hook_DXGICreateSwapChain);
		//ga_error("After Hooking CreateSwapChain\n");
	}
...
hook_D3D11ClearDepthStencilView(ID3D11DeviceContext * This, ID3D11DepthStencilView* pDepthStencilView,
	UINT ClearFlags,
	FLOAT Depth,
	UINT8 Stencil ) {	
	HRESULT hr;	
	dpipe_buffer_t *data_d;
	vsource_frame_t *frame_d;
	ID3D11DepthStencilView* curDSV;	
	This->OMGetRenderTargets(1, NULL, &curDSV);
	//ga_error("null resource\n");	
	ID3D11Resource * pSrcResource_d = NULL;
	if(curDSV == NULL)
		ga_error("curDSV is null \n");	
	ID3D11Device *pDevice = NULL;
	This->GetDevice(&pDevice);
	curDSV->GetResource(&pSrcResource_d);
	//ga_error("got resource\n");	
	ID3D11Texture2D *pSrcBuffer_d = (ID3D11Texture2D *)pSrcResource_d;
	ID3D11Texture2D *pDstBuffer_d = NULL;
		
	
	D3D11_TEXTURE2D_DESC desc_d;
	pSrcBuffer_d->GetDesc(&desc_d);
	
	if(desc_d.Width==encoder_width && desc_d.Height==encoder_height){
		desc_d.BindFlags = 0;
		desc_d.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
		desc_d.Usage = D3D11_USAGE_STAGING;		
	
		hr = pDevice->CreateTexture2D(&desc_d, NULL, &pDstBuffer_d);
		//ga_error("created texture\n");	
		if (FAILED(hr)) {
			OutputDebugString("Failed to create depth buffer");
			//assert(exp_state == exp_none);
		}
		This->CopyResource(pDstBuffer_d, pSrcBuffer_d);
		//ga_error("copied texture\n");	
		D3D11_MAPPED_SUBRESOURCE mapped_screen_d;
		hr = This->Map(pDstBuffer_d, 0, D3D11_MAP_READ, 0, &mapped_screen_d);
		//ga_error("mapped buffer\n");	
		if (FAILED(hr)) {
			OutputDebugString("Failed to map from DeviceContext");
			//assert(exp_state == exp_none);
		}
		if(depth_format_set==0){
				depth_format_set=1;
				char format[10];
				sprintf_s(format,"%d",desc_d.Format);
				ga_conf_writev("d-format",format);
				ga_error("Got Description:%d, width:%d, height:%d!\n",desc_d.Format,desc_d.Width,desc_d.Height);
				// Number of bytes per line for the depth is different based on the game
				//d-bytes is the number of bytes for a depth value per pixel
				//d-uf is the format of the depth: 0(u)-> UNORM , 1(f)->float
				switch(desc_d.Format){
					case DXGI_FORMAT_D16_UNORM:
					case DXGI_FORMAT_R16_TYPELESS:						
						ga_conf_writev("d-bytes","2");
						ga_conf_writev("d-uf","0");
						break;
					case DXGI_FORMAT_D32_FLOAT:							
					case DXGI_FORMAT_R32_TYPELESS:					
						ga_conf_writev("d-bytes","4");
						ga_conf_writev("d-uf","1");
						break;
					...
					case DXGI_FORMAT_R32G8X24_TYPELESS:
						ga_conf_writev("d-bytes","8");
						ga_conf_writev("d-uf","1");
						break;
				}
				//vsource_init_d(game_width,game_height);
			}		
		do {
		
			unsigned char *src_d, *dst_d;					
			if(rc){
				data_d = dpipe_get(g_pipe_d[0]);
				frame_d = (vsource_frame_t*) data_d->pointer;
				frame_d->pixelformat = PIX_FMT_RGBA;
				frame_d->realwidth = desc_d.Width;
				frame_d->realheight = desc_d.Height;
				frame_d->realstride = mapped_screen_d.RowPitch;						
				frame_d->realsize = frame_d->realwidth * frame_d->realstride;
				frame_d->linesize[0] = frame_d->realstride;//frame->stride;
				src_d = (unsigned char*) mapped_screen_d.pData;	
				dst_d = (unsigned char*) frame_d->imgbuf;
			}					
	
			for (int i = 0; i < desc_d.Height; i++) {							
				if(rc){
					CopyMemory(dst_d, src_d, frame_d->realstride/*frame->stride*/);	
					src_d += mapped_screen_d.RowPitch;				
					dst_d += frame_d->realstride;//frame->stride;				
				}
			}
		
		} while(0);
		if(rc)
			dpipe_store(g_pipe_d[0], data_d);
		
		This->Unmap(pDstBuffer_d, 0);
	...
	pDevice->Release();
	if(pSrcResource_d!=NULL)
		pSrcResource_d->Release();
	if(pSrcBuffer_d!=NULL)
		pSrcBuffer_d->Release();	
	if(pDstBuffer_d!=NULL)
		pDstBuffer_d->Release();
	//ga_error("bye from stencil\n");	
	pClearDepthStencilView(This,pDepthStencilView, ClearFlags,	Depth, Stencil);	
	return;
}

bool
check_dx_device_version(IDXGISwapChain * This, const GUID IID_target) {
	//ga_error("Device Version Check!\n");
	IUnknown *pDevice = NULL;
	HRESULT hr;

	if (dx_version != dx_none)
		return FALSE;  // this device has been checked
	//ga_error("Pass 1st Device Version Check!\n");
	hr = This->GetDevice(IID_target, (void**)&pDevice);
	//ga_error("Pass 2nd Device Version Check!\n");
	if (FAILED(hr) || pDevice == NULL) {  // failed to get this device 
		//ga_error("Fail Device Version Check!\n");
		if(pDevice != NULL)
			pDevice->Release();
		//ga_error("Relsease Fail Device Version Check!\n");
		return FALSE; 
	}
	//ga_error("Pass Device Version Check!\n");
	pDevice->Release();
	return TRUE;
}
...
{	
	...
	//ga_error("video source init success!\n");
	...
	//ga_error("dimension check pass!\n");
	//
	if (enable_server_rate_control && ga_hook_video_rate_control() < 0)
		return hr;
	//ga_error("rate control check pass!\n");
	...
	// d11
		...
		//ga_error("Inside DX11:%d,%lld MB!\n",frames,physMemUsedByMe/(1024*1024));
		//ga_error("Inside DX11!\n");
		void *ppDevice;	
		This->GetDevice(IID_ID3D11Device, &ppDevice);
		/*if(ppDevice == NULL)
			ga_error("Device Pointer Is Null!\n");
		ga_error("Got Device!\n");*/
		ID3D11Device *pDevice = (ID3D11Device*) ppDevice;
		
		This->GetDevice(IID_ID3D11DeviceContext, &ppDevice);
		/*if(ppDevice == NULL)
			ga_error("Device Context Pointer Is Null!\n");		*/
		//ga_error("Got Device Context!\n");
		ID3D11DeviceContext *pDeviceContext = (ID3D11DeviceContext *) ppDevice;				
		//ga_error("Context cast success!\n");
		ID3D11RenderTargetView *pRTV = NULL;
		//ga_error("Render Target declaration success!\n");
		//ID3D11DepthStencilView *pRTV_d = NULL;
		ID3D11Resource *pSrcResource = NULL;
		//ga_error("Resource declaration success!\n");
		//ID3D11Resource *pSrcResource_d = NULL;
		if(pDeviceContext == NULL){			
			//ga_error("Device Context Is Null!\n");
			pDevice->GetImmediateContext(&pDeviceContext);
		}

		/*if(depth_hooked==0 && mode==CAVE_D){
			uintptr_t* pInterfaceVTable = (uintptr_t*)*(uintptr_t*)pDeviceContext;				
			pClearDepthStencilView = (TD3D11ClearDepthStencilView)pInterfaceVTable[53];	
			if(pClearDepthStencilView==NULL)
				ga_error("can't hook depth stencil view\n");
			ga_hook_function("ID3D11DeviceContext::ClearDepthStencilView ", pClearDepthStencilView, hook_D3D11ClearDepthStencilView);
			depth_hooked = 1;
		}*/
		//pDeviceContext->OMGetRenderTargets(1, &pRTV, &pRTV_d);
		pDeviceContext->OMGetRenderTargets(1, &pRTV, NULL);
		//ga_error("Got Render Targets!\n");
		if(pRTV==NULL)
			ga_error("Render Target Is Null!\n");
		pRTV->GetResource(&pSrcResource);
		//ga_error("Got Resource!\n");
		//pRTV_d->GetResource(&pSrcResource_d);
		...
		//	//assert(exp_state == exp_none);
		//}
		//pDeviceContext->CopyResource(pDstBuffer_d, pSrcBuffer_d);

		//D3D11_MAPPED_SUBRESOURCE mapped_screen_d;
		//hr = pDeviceContext->Map(pDstBuffer_d, 0, D3D11_MAP_READ, 0, &mapped_screen_d);
		//if (FAILED(hr)) {
		//	OutputDebugString("Failed to map from DeviceContext");
		//	//assert(exp_state == exp_none);
		//}

		//if(ga_conf_readv("d-format",NULL,0)==NULL){
		//	char format[10];
		//	sprintf_s(format,"%d",desc_d.Format);
		//	ga_conf_writev("d-format",format);
		//	ga_error("depth %d\n",desc_d.Format);
		...
		do {
			...
			frame->realwidth = desc.Width;
			frame->realheight = desc.Height;
			frame->realstride = mapped_screen.RowPitch;						
			frame->realsize = frame->realwidth * frame->realstride;
			frame->linesize[0] = frame->realstride;//frame->stride;

			//ga_error("depth: before get\n");
			//if(rc){
			//	data_d = dpipe_get(g_pipe_d[0]);
			//	frame_d = (vsource_frame_t*) data_d->pointer;
			//	frame_d->pixelformat = PIX_FMT_RGBA;
			...
			//ga_error("Copied Frame!\n");
			//FILE* file;
			//FILE* file_d;
			//if(cnt==60){
			//		ga_error("Start Saving!\n");
			//		//file = fopen( "C:\\Users\\mhegazy\\Desktop\\temp.png", "wb" );
			//		file_d = fopen( "C:\\Users\\mhegazy\\Desktop\\temp_d.bin", "wb" );
			//}
			//ga_error("depth: before copy mem\n");
			for (i = 0; i < encoder_height; i++) {				
				CopyMemory(dst, src, frame->realstride/*frame->stride*/);																	
				src += mapped_screen.RowPitch;
				dst += frame->realstride;//frame->stride;				
				
				//convert_depth(i,src_d,desc_d.Format);
				//if(cnt==60){					
					//fwrite( dst, 1, frame->realstride, file );					
					//fwrite( depths[i], sizeof(depths[i]), frame->realstride, file_d );					
				//}
				//if(rc){
				//	CopyMemory(dst_d, src_d, frame_d->realstride/*frame->stride*/);	
				//	src_d += mapped_screen_d.RowPitch;				
		...
		dpipe_store(g_pipe[0], data);
		//ga_error("depth: before store\n");
		/*if(rc)
			dpipe_store(g_pipe_d[0], data_d);*/
		

		pDeviceContext->Unmap(pDstBuffer, 0);
		//pDeviceContext->Unmap(pDstBuffer_d, 0);
