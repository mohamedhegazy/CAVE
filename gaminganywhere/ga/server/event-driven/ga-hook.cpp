...
hook_dxgi(const char *hook_type, const char *hook_method) {
	HMODULE hMod;
	//
	//ga_error("Hooking DXGI functions!\n");
	...
	//
	pCreateDXGIFactory1 = (TCreateDXGIFactory) GetProcAddress(hMod, "CreateDXGIFactory1");
	if (pCreateDXGIFactory1 == NULL) {
		ga_error("GetProcAddress(CreateDXGIFactory1) failed.\n");
		return -1;
	}
	//
	ga_hook_function("CreateDXGIFactory1", pCreateDXGIFactory1, hook_CreateDXGIFactory1);
