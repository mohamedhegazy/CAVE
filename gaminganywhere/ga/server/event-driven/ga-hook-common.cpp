...
//The RC module will take the formatted depth from filter-d2float module and the raw frame from the encoder module (to predict ROI) and 
//output a map of QP for the encoder module
...
static ga_module_t *m_filter,*m_filter_d, *m_vencoder, *m_asource, *m_aencoder, *m_ctrl, *m_server, *m_rc;
...
vsource_init(int width, int height) {
	...
	if(ga_conf_readbool("content-aware", 0) != 0){
		for(unsigned int iid = 0; iid < SOURCES; iid++) {
			char srcpipename[64], dstpipename[64];				
			snprintf(dstpipename, sizeof(dstpipename), imagepipefmt_d, iid);
			g_pipe_d[iid] = dpipe_create(iid, dstpipename, POOLSIZE,
						sizeof(vsource_frame_t) + video_source_mem_size(iid));
				if(g_pipe_d[iid] == NULL) {
					ga_error("Depth: create dst-pipeline failed (%s).\n", dstpipename);
					goto init_failed;
				}
				dpipe_buffer_t *data = NULL;
				for(data = g_pipe_d[iid]->in; data != NULL; data = data->next) {
					if(vsource_frame_init(iid, (vsource_frame_t*) data->pointer) == NULL) {
						ga_error("Depth : init frame failed for %s.\n", dstpipename);
						goto init_failed;
					}
				}
				if(video_source_add_pipename(iid, dstpipename)==NULL)
					ga_error("adding depth pipe returned null\n");
		}
	}
	vsource_initialized = 1;
	return 0;
init_failed:
	ga_error("Video Init failed\n");
	for(unsigned iid = 0; iid < SOURCES; iid++) {
		if(g_pipe_d[iid] != NULL)
			dpipe_destroy(g_pipe_d[iid]);
		g_pipe_d[iid] = NULL;
	}		
	return -1;
}
...
load_modules() {
	...
	if(ga_conf_readbool("content-aware", 0) != 0){
		...
		snprintf(module_path, sizeof(module_path),
			BACKSLASHDIR("%s/mod/rc", "%smod\\rc"),
			ga_root);
		if((m_rc = ga_load_module(module_path, "content_aware_rc_")) == NULL)
			return -1;
		//ga_error("before d2float load\n");
		snprintf(module_path, sizeof(module_path),
			BACKSLASHDIR("%s/mod/filter-d2float", "%smod\\filter-d2float"),
			ga_root);
		if((m_filter_d = ga_load_module(module_path, "filter_D2FLOAT_")) == NULL)
			return -1;
	}
	//
	snprintf(module_path, sizeof(module_path),
		BACKSLASHDIR("%s/mod/encoder-%s", "%smod\\encoder-%s"),
		ga_root,type);
...
init_modules() {
	...
	if(ga_conf_readbool("content-aware", 0) != 0){
		//ga_error("before d2float init\n");
		ga_init_single_module_or_quit("filter_d", m_filter_d, (void*) filter_param_d);
		//ga_error("before rc init\n");
	...
	/*if(ga_conf_readbool("content-aware", 0) != 0){
		ga_init_single_module_or_quit("content-aware-rc", m_rc, NULL);
	}*/
	return 0;
}
...
run_modules() {
	...
	if(ga_conf_readbool("content-aware", 0) != 0){
		if(m_filter_d->start(filter_param_d) < 0)	exit(-1);
		//ga_error("after start filter depth \n");
