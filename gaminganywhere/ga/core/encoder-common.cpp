...
static ga_module_t *rc = NULL;	/**< RC instance */
...
static void *rc_param = NULL;	/**< Vieo encoder parameter */
...
/**
 * Register a RC module.
 *
 * @param m [in] Pointer to the rc module.
 * @param param [in] Pointer to the rc parameter.
 * @return Currently it always returns 0.
 *
 * The rc module is launched when a client is conneted.
 * The \a param is passed to the rc module when the module is launched.
 */
int
encoder_register_rc(ga_module_t *m, void *param) {
	if(rc != NULL) {
		ga_error("rc: warning - replace rc%s with %s\n",
			rc->name, m->name);
	}
	rc = m;
	rc_param = param;
	ga_error("rc: %s registered\n", m->name);
	return 0;
}
...
/**
 * Get the currently registered rc module.
 *
 * @return Pointer to the rc module, or NULL if not registered.
 */
ga_module_t *
encoder_get_rc() {
	return rc;
...
encoder_register_client(void /*RTSPContext*/ *rtsp) {
	...
	if(encoder_clients.size() == 0) {
		...
		if(vencoder != NULL && vencoder->init != NULL) {
			...
			if(ga_conf_readbool("content-aware", 0) != 0 && rc!=NULL && rc->init!=NULL){
				if(rc->init(rc_param) < 0) {					
					ga_error("rc: init failed.\n");					
					exit(-1);
				}
			}
		}
		...
		if(vencoder != NULL && vencoder->start != NULL) {
			...
			if(ga_conf_readbool("content-aware", 0) != 0 && rc!=NULL && rc->start!=NULL){
				if(rc->start(rc_param) < 0) {
					pthread_rwlock_unlock(&encoder_lock);
					ga_error("rc: start failed.\n");
					threadLaunched = false;
					exit(-1);
				}
			}
...
encoder_unregister_client(void /*RTSPContext*/ *rtsp) {
	...
	if(encoder_clients.size() == 0) {
		...
		if(rc != NULL && rc->stop != NULL)
			rc->stop(rc_param);
		if(rc != NULL && rc->deinit != NULL)
			rc->deinit(rc_param);
#ifdef ENABLE_AUDIO
